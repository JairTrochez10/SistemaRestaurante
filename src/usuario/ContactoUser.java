...
public class ContactoUser {
    public boolean insertar(Contacto contacto){
        ...
        {
            ps.setString(1, contacto.usuario);
            ps.setString(2, contacto.contrasena);
            ps.setString(3, contacto.rol);
	...
	try(Connection cn=Conexion.getConnection();
		PreparedStatement ps=cn.prepareStatement(sql)){
		ps.setString(1, contacto.getUsuario());
		ps.setString(2, contacto.getContrasena());
		ps.setString(3, contacto.getRol());
        ...
        try (Connection cn = Conexion.getConnection();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    contacto.usuario = rs.getString("usuario");
                    contacto.contrasena = rs.getString("contrasena");
                    contacto.rol = rs.getString("rol");
                    return contacto;
                }
            }
...
public ArrayList<Contacto> listarTodos() {
    ...
    String sql = "SELECT id, usuario, contrasena, rol FROM usuarios";
    ...
    try {
        ...
        while (rs.next()) {
            ...
            Contacto c = new Contacto(
                ...
                rs.getString("usuario"),   // obtiene el nombre
                rs.getString("contrasena"), // obtiene el teléfono
                rs.getString("rol") // obtiene la dirección
            );
...
public ArrayList<Contacto> buscarPorNombre(String nombre) {
    ...
    String sql = "SELECT id, usuario, contrasena, rol FROM usuarios WHERE usuario LIKE ?";
